...
from utils.trainval_func import site_evaluation, SaveCheckPoint, test_func
...
def test_on_target_domain(model_dict, dataloader_dict, target_domain, source_domains, log_file, comm_round, note=""):
    ...
    for source_domain in source_domains:
        ...
        log_file.info(
            f'{note} Round: {comm_round:3d} | Source Domain: {source_domain} | Target Domain: {target_domain} | Acc: {acc * 100:.2f}%')
...
def main():
    ...
    for i in range(start_round, args.comm + 1):
        ...
        for domain_name in source_domain_list:
            ...
            site_results_before_avg[domain_name] = site_evaluation(dual_model_dict[domain_name],
        ...
        if before_best_acc > best_target_acc:
            ...
            log_file.info(
                f'🏆 New Best Model for Target Domain {args.test_domain}! Round: {i}, Source Domain: {before_best_domain}, Phase: before_avg, Acc: {before_best_acc * 100:.2f}%')
        ...
        for domain_name in source_domain_list:
            site_results_after_avg[domain_name] = site_evaluation(dual_model_dict[domain_name],
                                                                  dataloader_dict[domain_name]['val'])
        ...
        if after_best_acc > best_target_acc:
            ...
            SaveCheckPoint(args, single_model_dict[after_best_domain], args.comm,
                           ...
                           note=f'best_target_{args.test_domain}_single')
            log_file.info(
                f'🏆 New Best Model for Target Domain {args.test_domain}! Round: {i}, Source Domain: {after_best_domain}, Phase: after_avg, Acc: {after_best_acc * 100:.2f}%')

        # Print summary of the best performance
        log_file.info(f"\n📊 Current Best Performance Summary:")
        info = best_target_info
        log_file.info(
            f"Target Domain: {args.test_domain} | Best Round: {info['round']} | Best Source Domain: {info['source_domain']} | Phase: {info['phase']} | Acc: {info['acc'] * 100:.2f}%")
    ...
    info = best_target_info
    log_file.info(
        f"Best Target Domain Accuracy: {info['acc'] * 100:.2f}% (Round {info['round']}, Source Domain: {info['source_domain']}, Phase: {info['phase']})")
