...
class Scaffold(Optimizer):
    ...
    def step(self, c_ci):
        ...
        for p, control_c_ci in zip(group['params'], c_ci):
            ...
            d_p.add_(control_c_ci.data, alpha=1.0)
...
def GenParamList(model_params):
    new_list = []
    for param in model_params:
        new_list.append(param.clone())
    return new_list

def GenZeroParamList(model_params):
    c = GenParamList(model_params)
    for i in c:
        i.data.zero_()
    return c
...
def UpdateLocalControl(c, ci, global_model_params, local_model_params, K):
    '''
    ...
    '''
    param_num = len(c)
    x = GenParamList(global_model_params)
    yi = GenParamList(local_model_params)
